...
class Command(BaseCommand):
    ...
    def send_test_email(self, template_name, options):
        ...
        context = {'name': 'Usuario de Prueba', 'student_name': 'Estudiante Test', 'amount': 100.50, 'due_date': '31/12/2025'}
        ...
        success = email_service.send_email(template_name=template_name, recipients=recipient, subject=subject, context=context)
    ...
    def send_birthday_emails(self):
        today = date.today()
        birthday_students = Student.objects.filter(birth_date__month=today.month, birth_date__day=today.day, active=True).prefetch_related('parents')
        ...
        for student in birthday_students:
            ...
            if not parent:
                failed += 1
                continue
            success = email_service.send_email(template_name='happy_birthday', recipients=parent.email, subject=f'¡Feliz Cumpleaños {student.first_name}!', context={'name': student.first_name})
    ...
    def send_monthly_reports(self):
        ...
        for parent in parents:
            ...
            context = {'parent_name': parent.full_name, 'students': [{'name': s.full_name, 'group': s.group.group_name if s.group else 'Sin grupo'} for s in students], 'total_students': students.count()}
            success = email_service.send_email(template_name='monthly_report', recipients=parent.email, subject='Reporte Mensual - Five a Day', context=context)
            sent += 1 if success else 0
            failed += 0 if success else 1
        self.stdout.write(self.style.SUCCESS(f'Resultado: {sent} enviados, {failed} fallidos'))
    ...
    def send_fun_friday_emails(self, options):
        ...
        try:
            ...
            raise CommandError('Formato de fecha inválido')
        ...
        success = send_fun_friday_email(recipients=recipient_emails, day_name=DAYS_ES[event_date.weekday()], day_number=event_date.day, month=MONTHS_ES[event_date.month - 1], start_time=start_time, end_time=end_time, activity_description=options['activity'], minimum_age=options.get('min_age', 4), maximum_age=options.get('max_age', 12), meeting_point=options.get('meeting_point'), event_image_path=options.get('event_image'))
        self.stdout.write(self.style.SUCCESS(f'Enviado a {len(recipient_emails)} padres') if success else self.style.ERROR('Error'))
    ...
    def send_payment_reminder_emails(self, options):
        ...
        success = send_payment_reminder_email(recipients=recipient_emails, payment_start_day_name=DAYS_ES[0], payment_start_day_number=int(options.get('payment_start', 1)), payment_end_day_name=DAYS_ES[4], payment_end_day_number=int(options.get('payment_end', 5)), month=options['month'], iban_number=options.get('iban', ''), reduced_price_cheque_idioma=options.get('cheque_idioma_price', '40'), telephone_number_bizum=options.get('bizum_phone', ''))
        self.stdout.write(self.style.SUCCESS(f'Enviado a {len(recipient_emails)} padres') if success else self.style.ERROR('Error'))
    ...
    def send_vacation_closure_emails(self, options):
        ...
        parents = Parent.objects.filter(email__isnull=False, children__active=True).exclude(email='').distinct()
        recipient_emails = list(parents.values_list('email', flat=True))
        success = send_vacation_closure_email(recipients=recipient_emails, start_closure_day_name=DAYS_ES[start_date.weekday()], start_closure_day_number=start_date.day, end_closure_day_name=DAYS_ES[end_date.weekday()], end_closure_day_number=end_date.day, month_closure=MONTHS_ES[start_date.month - 1], closure_reason=options['reason'], reopening_day_name=DAYS_ES[reopen_date.weekday()], reopening_day_number=reopen_date.day, month_reopening=MONTHS_ES[reopen_date.month - 1])
        self.stdout.write(self.style.SUCCESS(f'Enviado a {len(recipient_emails)} padres') if success else self.style.ERROR('Error'))
    ...
    def send_tax_certificate_email_cmd(self, options):
        ...
        if options.get('recipient'):
            ...
            self.stdout.write(self.style.SUCCESS(f'Certificado enviado a {parent.email}') if success else self.style.ERROR('Error'))
        else:
            results = send_all_tax_certificates(year)
            self.stdout.write(self.style.SUCCESS(f'Resultado: {results["sent"]} enviados, {results["skipped"]} omitidos, {results["failed"]} fallidos'))
    ...
    def send_quarterly_receipt(self, options):
        ...
        success = send_quarterly_receipt_email(parent_email=parent.email, student_name=student.full_name, month_1=months[0].strip(), month_2=months[1].strip(), month_3=months[2].strip())
