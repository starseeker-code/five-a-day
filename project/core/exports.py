...
def build_students_sheet(ws) -> None:
    """Populate *ws* with the Estudiantes table (student + parent join)."""
    _style_header(ws, [
...
def build_enrollments_sheet(ws) -> None:
    """Populate *ws* with the Matrículas table (enrollment + student join)."""
    _style_header(ws, [
    ...
    for e in qs:
        ws.append([
            ...
            float(e.enrollment_amount),
            float(e.discount_percentage),
            float(e.final_amount),
...
def build_payments_sheet(ws) -> None:
    """Populate *ws* with the Pagos table (payment + student + parent join)."""
    _style_header(ws, [
        "ID", "Estudiante - Nombre", "Estudiante - Apellidos",
        "Tutor - Nombre", "Tutor - Apellidos", "Tutor - DNI",
...
def build_database_workbook() -> openpyxl.Workbook:
    """
    Return a Workbook with three sheets: Estudiantes, Matrículas, Pagos.
    This is the main export for /database — call wb.save(response) to stream it.
    """
